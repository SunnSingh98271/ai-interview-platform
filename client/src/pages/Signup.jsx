    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-900 px-4">
      <div className="bg-gray-800 p-8 rounded-lg w-full max-w-md">
        ...
        <form onSubmit={handleSubmit}>
          <input type="text" placeholder="Full Name" className="w-full p-3 mb-4 rounded bg-gray-700 text-white" value={name} onChange={e=>setName(e.target.value)} required />
          ...
          <button type="submit" className="w-full bg-purple-600 py-3 rounded font-semibold">Sign Up</button>
        </form>
        <p className="text-gray-400 text-center mt-4">Already have an account? <Link to="/login" className="text-purple-400">Login</Link></p>
      </div>
    </div>
  );
}